...
class _BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        return out
def resnet32():
    return _ResNet(_BasicBlock, [5, 5, 5])
...
class BCLModel_32(nn.Module):
    def __init__(self, num_classes=10, name='resnet50', head='mlp', use_norm=True, feat_dim=128, return_features=False):
        ...
        if head == 'mlp':
            self.head = nn.Sequential(nn.Linear(64, dim_in), nn.BatchNorm1d(dim_in), nn.ReLU(inplace=True),
        ...
        if (use_norm):
            ...
            self.head_fc = nn.Sequential(nn.Linear(64, dim_in), nn.BatchNorm1d(dim_in), nn.ReLU(inplace=True),
                                         nn.Linear(dim_in, feat_dim))
        else:
